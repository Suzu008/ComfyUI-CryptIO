import os
import hashlib
import numpy as np
import torch
from PIL import Image, ImageSequence, ImageOps
from io import BytesIO

import folder_paths  # pyright: ignore[reportMissingImports]
import node_helpers  # pyright: ignore[reportMissingImports]
from comfy_api.latest import io

from .utils import _key_manager
from .utils.crypto_utils import decrypt_data_hybrid


class UploadImageCryptIO(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        input_dir = folder_paths.get_input_directory()
        files = [f for f in os.listdir(input_dir) if os.path.isfile(os.path.join(input_dir, f))]
        encrypted_files = [
            f
            for f in files
            if f.endswith(
                (
                    ".png.encrypted",
                    ".jpg.encrypted",
                    ".jpeg.encrypted",
                    ".webp.encrypted",
                    ".avif.encrypted",
                )
            )
        ]
        all_files = sorted(set(encrypted_files))

        return io.Schema(
            node_id="UploadImageCryptIO",
            display_name="Upload Image 🔒",
            category="CryptIO🔒",
            description="Upload and decrypt image files encrypted with server public key",
            inputs=[
                io.Combo.Input("image", options=all_files, extra_dict={"image_upload": True}, tooltip="Select an encrypted image file to upload and decrypt"),
            ],
            outputs=[
                io.Image.Output(display_name="image"),
                io.Mask.Output(display_name="mask"),
            ],
        )

    @classmethod
    def execute(cls, image):
        if not isinstance(image, str) or not image.endswith(".encrypted"):
            raise ValueError("Invalid image input, expected encrypted image file")

        image_path = folder_paths.get_annotated_filepath(image)

        with open(image_path, "rb") as f:
            encrypted_combined = f.read()

        decrypted_data = decrypt_data_hybrid(encrypted_combined, _key_manager.server_private_key)

        img = Image.open(BytesIO(decrypted_data))

        output_images = []
        output_masks = []
        w, h = None, None

        for i in ImageSequence.Iterator(img):
            i = node_helpers.pillow(ImageOps.exif_transpose, i)

            if i.mode == "I":
                i = i.point(lambda i: i * (1 / 255))
            image_rgb = i.convert("RGB")

            if len(output_images) == 0:
                w = image_rgb.size[0]
                h = image_rgb.size[1]

            if image_rgb.size[0] != w or image_rgb.size[1] != h:
                continue

            image_array = np.array(image_rgb).astype(np.float32) / 255.0
            image_tensor = torch.from_numpy(image_array)[None,]

            if "A" in i.getbands():
                mask = np.array(i.getchannel("A")).astype(np.float32) / 255.0
                mask = 1.0 - torch.from_numpy(mask)
            elif i.mode == "P" and "transparency" in i.info:
                mask = np.array(i.convert("RGBA").getchannel("A")).astype(np.float32) / 255.0
                mask = 1.0 - torch.from_numpy(mask)
            else:
                mask = torch.zeros((64, 64), dtype=torch.float32, device="cpu")

            output_images.append(image_tensor)
            output_masks.append(mask.unsqueeze(0))

        if len(output_images) > 1 and img.format not in ["MPO"]:
            output_image = torch.cat(output_images, dim=0)
            output_mask = torch.cat(output_masks, dim=0)
        else:
            output_image = output_images[0]
            output_mask = output_masks[0]

        return io.NodeOutput(output_image, output_mask)

    @classmethod
    def fingerprint_inputs(cls, image):
        if isinstance(image, str) and not image.startswith("ENCRYPTED:"):
            image_path = folder_paths.get_annotated_filepath(image)
            m = hashlib.sha256()
            with open(image_path, "rb") as f:
                m.update(f.read())
            return m.digest().hex()
        return float("NaN")

    @classmethod
    def validate_inputs(cls, image):
        if isinstance(image, str) and image.endswith(".encrypted"):
            if not folder_paths.exists_annotated_filepath(image):
                return f"Invalid encrypted image file: {image}"
            return True

        if not folder_paths.exists_annotated_filepath(image):
            return f"Invalid image file: {image}"

        return True
